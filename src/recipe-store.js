...
import { cloneRecipeList } from './util-funcs/clone-recipes';
...
function reduxServerData(state, action) {
  ...
  const recipes_other = cloneRecipeList(state.recipes_other);
  const recipes_kitchen = cloneRecipeList(state.recipes_kitchen);
  const google_other = state.google_other;
  show_previous = state.show_which;
  const show_which = state.show_which;
  const google_user_id = state.google_user_id;
...
function reduxSilentSignin(state, action) {
  ...
  if (user_info) {
    ...
    if (show_previous === SHOW_KITCHEN) {
      show_which = SHOW_KITCHEN;  // if user quickly went to kitchen, before silent-signin
    } else {
      show_which = SHOW_YOURS;   // since on main screen, flip to user's recipes as expected
    }
